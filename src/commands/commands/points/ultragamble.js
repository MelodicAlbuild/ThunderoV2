...
module.exports = class gambleCommand extends Command {
  constructor(client) {
    super(client, {
      name: 'ultragamble',
      aliases: [],
      usage: 'ultragamble <point count>',
      description: 'Gamble your points.',
      type: client.types.POINTS,
      examples: ['ultragamble 1000'],
      exclusive: true
    });
  }
  async run(message, args) {
      ...
      if(points < 100000000000000000000) {
          this.done(message.author.id)
          return this.sendErrorMessage(message, 1, `You must have 1+e21 points to Super Gamble!`)
      }
          ...
          setTimeout(async () => {
              ...
              if (outcome === "lose") {
                  const embed = new MessageEmbed()
                      ...
                      .setFooter(`Your points: ${points - amount * 4}.`, message.author.displayAvatarURL({dynamic: true}))
                  message.client.db.users.updatePoints.run({points: -amount * 4}, message.author.id, message.guild.id);
